...
{	/**** Write your kernel here ****/
    ...
    int bx = blockIdx.x;
    int by = blockIdx.y;
    int I_large = N*BSZ*by + N*(j+1) + BSZ*bx + i+1;
    int I_small = (N-2)*BSZ*by + (N-2)*j + BSZ*bx+i;

    int G_I = bx*BSZ+i;
    int G_J = by*BSZ+j;

    if ((G_J>N-3) || (G_I>N-3)) return; 
    RHS[I_small] = u[I_large] + alpha*dt/(2*h*h) * (u[I_large+1] + 
                    u[I_large-1] + u[I_large+N] +
                    u[I_large-N] - 4*u[I_large]);
    if (G_I == 0)
        RHS[I_small] += (alpha*dt/(2*h*h))*T_bound;
    if (G_J  == 0)
        RHS[I_small] += (alpha*dt/(2*h*h))*T_bound;
}
...
int main()
{
	...
	// Loop 
	dim3 dimBlock(BLOCKSIZE,BLOCKSIZE); // Total number of blocks?
	dim3 dimGrid(int((N-2-0.5)/BLOCKSIZE)+1,int((N-2-0.5)/BLOCKSIZE)+1); // Numer of threads per block?
