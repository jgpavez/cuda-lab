...
void MatMul(const Matrix A, const Matrix B, Matrix C)
{
	...
	// call kernel
        ...
        dim3 dimGrid(int(B.width - 0.5)/BLOCK_SIZE + 1,
                     int(A.height - 0.5)/BLOCK_SIZE + 1); // number of blocks?
...
{	
    ...
    for ( int i = 0; i < A.width; i++){
        result += A.elements[row*A.width + i] * B.elements[i*B.width + col];
    }
    C.elements[row*C.width + col] = result;
